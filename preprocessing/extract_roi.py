...
def video_to_frames(
    video_path: os.PathLike, output_path: os.PathLike, detector, predictor
) -> bool:
...
def extract_video_data(
    path: os.PathLike, detector, predictor, verbose: bool = True
) -> Optional[np.ndarray]:
    if verbose:
        print("\n{}".format(path))
...
def extract_mouth_on_frame(
    frame: np.ndarray, detector, predictor, idx: int
) -> Optional[np.ndarray]:
